...
def detect_viewing_preference_locally(user_text: str) -> str:
    """Detect explicit home/cinema answers without relying on Gemini.

    This is important after the agent asks: "בבית או בקולנוע?".
    The answer may be only one word, for example "בבית". In that case the
    current user text must override any conversation-context words such as
    "cinema", "current", or "screenings".
    """
    text = normalize_user_text(user_text).lower().strip()

    home_terms = [
        "בבית", "בית", "לצפייה בבית", "צפייה בבית", "סטרימינג",
        "טלוויזיה", "טלויזיה", "home", "at home", "streaming", "watch at home"
    ]
    cinema_terms = [
        "בקולנוע", "קולנוע", "בית קולנוע", "הקרנה", "הקרנות",
        "מוקרנים", "מוקרן", "עכשיו בקולנוע", "cinema", "theater", "theatre", "now playing"
    ]
...
def answer_home_recommendation(session_id: str, full_home_request: str) -> str:
    """Answer a home-viewing recommendation using CSV first, then Gemini fallback.

    This function must NOT call the current-cinema / Google Search flow. Home
    recommendations are not live showtime questions.
    """
    ...
    return final_answer(
        full_home_request,
        local,
        "home viewing recommendation from local CSV",
        status,
        allow_general_knowledge=(status == "data_missing"),
    )
...
def movie_agent(user_message: str, session_id: str = "default") -> str:
    ...
    if pending_viewing_request:
        ...
        # First handle explicit one-word answers locally. Do not ask Gemini to
        # classify "בבית" in a context that contains the word "cinema", because
        # that can accidentally route a home request to the current-cinema flow.
        local_preference = detect_viewing_preference_locally(original_message)
