...
def movie_agent(user_message: str, session_id: str = "local") -> str:
    ...
    if current_preference in {"home", "cinema"} and get_last_context(session_id):
    ...
    # Follow-up filters update the active recommendation using the last viewing
    # mode before generic detail-question routing sees words such as "אורך".
    last_viewing_mode = get_last_viewing_mode(session_id)
    if last_viewing_mode in {"home", "cinema"} and has_recommendation_intent(original_message):
        combined_request = merge_with_movie_context(session_id, original_message)
    ...
    if has_recommendation_intent(original_message):
        combined_request = merge_with_movie_context(session_id, original_message)
        effective_preference = current_preference
        if effective_preference not in {"home", "cinema"}:
            effective_preference = get_last_viewing_mode(session_id)
        if effective_preference in {"home", "cinema"}:
            return answer_recommendation(combined_request, session_id, effective_preference)
        save_pending_request(session_id, combined_request)
        return build_home_or_cinema_question(combined_request)
