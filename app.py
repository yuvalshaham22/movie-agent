...
def build_cinema_search_prompt(user_request: str) -> str:
    return (
        ...
        "Use a user-friendly format for each movie, for example: title, short explanation/summary, genre/style, and a note that exact showtimes depend on the user's local cinema. "
        "Do NOT use phrases like 'למה זה מתאים לבקשה', 'why it matches the request', or any internal reasoning labels. "
...
def movie_agent(user_message: str, session_id: str = "default") -> str:
    ...
    if pending_viewing_request:
        ...
        if has_cinema_intent(original_message):
        ...
        # If the answer was not clear, ask again and keep the original request.
        save_viewing_mode_pending(session_id, pending_viewing_request)
