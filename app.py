...
def should_ask_home_or_cinema(user_text: str) -> bool:
    """Ask home/cinema for any movie recommendation request that does not already specify viewing mode.
    ...
    Generic requests without preferences are handled earlier by should_ask_for_recommendation_details.
    """
    text = normalize_user_text(user_text).lower()
    if not has_recommendation_intent(text):
        return False
...
def build_home_or_cinema_question(user_text: str) -> str:
    ...
    return (
        ...
        + details_text +
        "\n\nאפשר לבחור:\n"
        "• בבית — אני אחפש קודם במאגר הסרטים שלנו, ואם חסר מידע אשלים מאחורי הקלעים.\n"
...
9. End with a friendly follow-up question: "רוצה שאפרט על אחד הסרטים עם שחקנים, מגבלת גיל, שנת יציאה או סרטי המשך?"
""".strip()
...
def movie_agent(user_message: str, session_id: str = "default") -> str:
    ...
    if msg in ["hi", "hello", "hey", "שלום", "היי", "הי"]:
        local = (
            ...
            "אפשר לכתוב לי חופשי מה בא לך לראות, ואני אנסה להבין את הבקשה ולדייק את ההמלצה.\n\n"
            "אני קודם בודק את הדאטה המקומי שלנו, ורק אם חסר מידע — למשל מידע עדכני על סרטים בקולנוע — "
            "אני משלים אותו מאחורי הקלעים בעזרת Gemini.\n\n"
            "אם הבקשה תהיה כללית מדי, אשאל אותך כמה שאלות קצרות כדי להבין מה מתאים לך."
        )
    ...
    if pending_viewing_request:
        ...
        if preference == "cinema" or answer_routing.get("needs_current_external_info"):
            full_cinema_request = answer_routing.get("normalized_query_hebrew") or combined_request
            save_last_cinema_request(session_id, full_cinema_request)
            return query_gemini_with_web_search(build_cinema_search_prompt(full_cinema_request))

        if preference == "home":
            state = get_state(session_id)
            state["last_viewing_mode"] = "home"
            full_home_request = answer_routing.get("normalized_query_hebrew") or combined_request
            state["last_recommendation_request"] = full_home_request
            # Use the normalized full request, so Gemini can correct typos such as
            # "עימה" -> "אימה" and preserve runtime/age constraints before the CSV search.
