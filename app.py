...
app.config["SECRET_KEY"] = os.getenv("FLASK_SECRET_KEY", "movie-agent-local-session-v1")
try:
...
def find_title_in_message(message: str) -> Optional[str]:
    ...
    patterns = [
        ...
        r"על מה הסרט\s+(.+)",
        r"תקציר\s+(?:של|על|ל)\s+(.+)",
        r"(?:תן|תני|אפשר|אשמח|אני רוצה|בא לי)?\s*(?:לי\s+)?תקציר\s+(?:של|על|ל)\s+(.+)",
...
def extract_recommended_titles_from_answer(answer: str) -> List[str]:
    """Extract titles from recommendation answers so follow-up questions can refer back."""
    titles: List[str] = []
    for raw_line in str(answer or "").splitlines():
        line = raw_line.strip()
        if not line:
            continue
        # Expected cinema/local recommendation format starts with 🎬 Title.
        match = re.match(r"^🎬\s*(.+)$", line)
        if not match:
            continue
        title = match.group(1).strip(" .:-")
        # Keep only the title, not appended labels if the model adds them.
        title = re.split(r"\s+(?:\||-|–)\s+(?:ז׳אנר|זאנר|genre|דירוג|rating)\b", title, maxsplit=1, flags=re.IGNORECASE)[0].strip()
        if title and title not in titles:
            titles.append(title)
    return titles[:8]


def save_recommended_titles(session_id: str, answer: str) -> None:
    titles = extract_recommended_titles_from_answer(answer)
    if titles:
        state = get_state(session_id)
        state["last_recommended_titles"] = "||".join(titles)
        # Default focus is the first recommendation, but explicit title questions will override it.
        state["focused_movie_title"] = titles[0]


def get_recommended_titles(session_id: str) -> List[str]:
    raw = get_state(session_id).get("last_recommended_titles", "")
    return [title for title in raw.split("||") if title]


def normalize_title_key(title: str) -> str:
    """Normalize movie titles for lightweight conversation-memory matching."""
    text = normalize_user_text(title or "").lower()
    text = re.sub(r"\([^)]*\)", " ", text)
    text = re.sub(r"[^a-z0-9א-ת]+", " ", text)
    return re.sub(r"\s+", " ", text).strip()


def title_was_recently_recommended(title: str, session_id: str) -> bool:
    key = normalize_title_key(title)
    if not key:
        return False
    for saved in get_recommended_titles(session_id):
        saved_key = normalize_title_key(saved)
        if key == saved_key or key in saved_key or saved_key in key:
            return True
    return False


def gemini_cinema_detail_fallback(user_text: str, movie_title: str, session_id: str) -> str:
    """Answer follow-up questions about a movie recommended in the CINEMA flow.

    This intentionally uses Google Search grounding and avoids the local CSV,
    because cinema availability is current data and local datasets can contain
    older/unrelated movies with the same or similar title.
    """
    recent_titles = get_recommended_titles(session_id)
    recent_context = "\n".join(f"- {title}" for title in recent_titles) if recent_titles else "- No saved titles"
    prompt = f"""
You are a Hebrew movie assistant.
The user previously chose CINEMA, so use Google Search grounding and answer only about the currently-screening movie/version.
Do NOT rely on older model knowledge or an old movie with the same/similar title.
First verify whether this exact movie is currently playing in cinemas in Israel or appears on a current cinema/showtime page.
If you cannot verify the exact currently-screening movie, say in Hebrew that you cannot verify it right now and ask for a city/cinema or another title.
Answer the user's current follow-up concisely in Hebrew.
If verified, include one short line at the end: מקור הקרנות: <direct URL>

Recent cinema recommendations shown to the user:
{recent_context}

Movie title to discuss:
{movie_title}

User question:
{user_text}
""".strip()
    answer = generate_with_gemini(prompt, use_google_search=True)
    if answer:
        return clean_user_answer(answer)
    return (
        "לא הצלחתי לאמת כרגע מידע עדכני על הסרט הזה בקולנוע. "
        "נסי שוב בעוד כמה דקות, או כתבי לי עיר/רשת קולנוע ואנסה לדייק את החיפוש."
    )


def title_from_ordinal_reference(user_text: str, session_id: str) -> Optional[str]:
    text = normalize_user_text(user_text).lower()
    titles = get_recommended_titles(session_id)
    if not titles:
        return None
    ordinal_map = {
        "ראשון": 1, "הראשון": 1, "1": 1, "אחד": 1,
        "שני": 2, "השני": 2, "2": 2, "שתיים": 2,
        "שלישי": 3, "השלישי": 3, "3": 3,
        "רביעי": 4, "הרביעי": 4, "4": 4,
        "חמישי": 5, "החמישי": 5, "5": 5,
    }
...
def model_fallback_list() -> List[str]:
    ...
    models = [configured]
    if env_fallbacks:
        # Render environment values should be comma-separated.
        # This also handles accidental line breaks or extra spaces.
        clean_fallbacks = env_fallbacks.replace("\n", ",").replace("\r", ",")
        models += [m.strip() for m in clean_fallbacks.split(",") if m.strip()]
    else:
...
def create_gemini_client(api_key: str):
    """Create Gemini client with a short network timeout.

    Render/Gunicorn kills a sync worker after the request timeout. Cinema
    searches use Google Search grounding and can be slower, so the Gemini SDK
    must timeout BEFORE Gunicorn does. This lets Flask return a normal message
    instead of the browser getting a failed /chat request.
    """
    if genai is None:
        return None
    timeout_ms = int(os.getenv("GEMINI_HTTP_TIMEOUT_MS", "20000"))
    if types is not None and hasattr(types, "HttpOptions"):
        try:
            ...
            # Older google-genai versions may not expose timeout on HttpOptions.
            pass
    try:
...
def generate_with_gemini(contents: str, use_google_search: bool = False) -> Optional[str]:
    ...
    for model_name in model_fallback_list():
        client = None
        try:
            # Important: create a fresh client for every fallback model.
            # After a 503/timeout the SDK client can be closed, and reusing it
            # causes: "Cannot send a request, as the client has been closed."
            ...
            # Google Search grounding connects Gemini to current web results.
            ...
            print(f"Gemini failed with {model_name}: {last_error}", flush=True)
    ...
    print(f"Gemini unavailable. Last error: {last_error}", flush=True)
    return None
...
def is_useful_cinema_answer(answer: str) -> bool:
    """Return True when Gemini produced a cinema-style answer.

    Google Search grounding often keeps the actual source URLs in grounding
    metadata instead of printing full URLs inside response.text. The previous
    version rejected every cinema answer that did not contain http:// or
    https://, which made valid grounded answers look like API failures.
    """
    text = str(answer or "").lower()
    if not text.strip():
        return False
    cinema_signals = [
        "מוקרן", "מוקרנים", "מוצג", "מוצגים", "הקרנה", "הקרנות",
        "קולנוע", "בתי קולנוע", "סינמה", "יס פלאנט", "רב חן", "לב", "מובילנד",
        "cinema", "theater", "theatre", "showtime", "now playing",
    ]
    movie_format_signals = ["🎬", "למה זה מתאים", "קולנוע/אזור", "ז׳אנר", "אורך הסרט"]
    return any(signal in text for signal in cinema_signals) or any(signal.lower() in text for signal in movie_format_signals)
...
Today is {today}. Use Google Search grounding to find movies that are actually playing in cinemas NOW, preferably in Israel because the conversation is in Hebrew.
The recommendations must match the user's mandatory constraints.
Do NOT recommend famous older movies unless the search results confirm they are currently screening now.
...
מקור מאומת: write the cinema/showtime source name or direct URL if available

Do not use markdown bullets or numbered lists. Include only movies that search results indicate are currently screening.
"""
    else:
...
""".strip()
    ...
    if answer:
        ...
        if not cinema:
            return cleaned
        # Grounded Gemini responses do not always print full URLs in response.text.
        # Accept cinema-style grounded answers even when the direct URL is absent.
        if is_useful_cinema_answer(cleaned):
            print("Gemini cinema answer accepted", flush=True)
            return cleaned
        print("Gemini cinema answer rejected because it did not look like a now-playing cinema answer", flush=True)
...
def answer_recommendation(user_text: str, session_id: str, viewing_mode: str) -> str:
    ...
    if viewing_mode == "cinema":
        ...
        if cinema_answer:
            ...
            save_recommended_titles(session_id, cinema_answer)
            return cinema_answer
        get_state(session_id)["last_cinema_search_failed"] = "1"
        return (
            "לא הצלחתי להשלים כרגע חיפוש עדכני של סרטים בקולנוע דרך Gemini. "
            "זה קורה בדרך כלל כשמודל Gemini עמוס או כשיש מגבלת שימוש/מכסה על ה־API key. "
            "ההעדפות שלך נשמרו, ואפשר לנסות שוב בעוד כמה דקות או לבדוק את המכסה ב־Google AI Studio."
        )
    ...
    if status == "data_found":
        save_recommended_titles(session_id, local_answer)
        return local_answer
    fallback_answer = gemini_movie_fallback(user_text, "movie recommendation", cinema=False)
    if fallback_answer:
        save_recommended_titles(session_id, fallback_answer)
    return fallback_answer


def answer_movie_detail(user_text: str, session_id: str) -> str:
    explicit_title = find_title_in_message(user_text)
    ordinal_title = title_from_ordinal_reference(user_text, session_id)
    focused_title = explicit_title or ordinal_title or get_focused_movie(session_id)

    # If the last flow was CINEMA, follow-up questions should stay in the
    # current cinema context and use Gemini + Google Search grounding.
    # This prevents an old/local dataset entry from answering a question about
    # a currently-screening movie with the same or similar title.
    if focused_title and get_last_viewing_mode(session_id) == "cinema":
        if title_was_recently_recommended(focused_title, session_id) or explicit_title or ordinal_title:
            save_focused_movie(session_id, focused_title)
            return gemini_cinema_detail_fallback(user_text, focused_title, session_id)
...
def movie_agent(user_message: str, session_id: str = "local") -> str:
    ...
    # A detail question about a specific/focused movie must win over genre detection.
    # Example: "תן לי תקציר על מת לצעוק (Scary Movie)" contains the word Scary,
    # but it is a title, not a new Horror recommendation request.
    if is_detail_question(original_message) and (
        find_title_in_message(original_message)
        or title_from_ordinal_reference(original_message, session_id)
        or get_focused_movie(session_id)
        or not has_recommendation_intent(original_message)
    ):
    ...
    # A NEW recommendation request should always start a fresh flow.
    # This prevents an old saved mode such as "cinema" from being reused when
    # the user starts over with: "אשמח להמלצה לסרט קומדיה".
    # Desired flow: save preferences -> ask home/cinema -> then search.
    if has_recommendation_intent(original_message) and current_preference == "unclear":
        state = get_state(session_id)
        state.pop("last_movie_context", None)
        state.pop("last_viewing_mode", None)
        state.pop("pending_viewing_mode", None)
        state.pop("waiting_for_cinema_preferences", None)
        state.pop("last_cinema_search_failed", None)
    ...
    # Detail questions about a focused/explicit/ordinal movie take priority over recommendation filters.
    if is_detail_question(original_message) and (
        find_title_in_message(original_message)
        or title_from_ordinal_reference(original_message, session_id)
...
def chat():
    ...
    CONVERSATION_STATE[session_id] = dict(session.get("conversation_state", {}))
    try:
    ...
    session["conversation_state"] = dict(CONVERSATION_STATE.get(session_id, {}))
