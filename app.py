...
def infer_viewing_preference_with_gemini(user_text: str) -> str:
    ...
    if text in VIEWING_INTENT_CACHE:
        return VIEWING_INTENT_CACHE[text]
...
""".strip()
    ...
    preference = {"HOME": "home", "CINEMA": "cinema"}.get(normalized, "unclear")
...
def get_movie_info(user_text: str, fallback_title: Optional[str] = None) -> Tuple[str, str]:
    ...
    if asks_sequels:
        base_title = re.sub(r"\s*\(\d{4}\)", "", str(row.get("title_no_year", movie_title)), flags=re.IGNORECASE).strip()
        related = df[
            df["title_no_year"].str.contains(re.escape(base_title), case=False, na=False)
            & (df.index != idx)
        ]["title_movielens"].head(8).tolist()
        if related:
            return (
                f"כן, קיימים סרטי המשך או סרטים קשורים ל-{movie_title}:\n"
                + "\n".join(f"- {title}" for title in related)
            ), "data_found"
        return f"לא מצאתי סרט המשך מזוהה ל-{movie_title}.", "data_found"
    if asks_runtime:
...
def movie_agent(user_message: str, session_id: str = "local") -> str:
    original_message = normalize_intent_with_gemini(str(user_message or "").strip())
    ...
    if pending_detail_question:
        ...
        return answer_movie_detail(combined_question, session_id)
