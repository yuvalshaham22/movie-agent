...
def answer_recommendation(user_text: str, session_id: str, viewing_mode: str) -> str:
    ...
    if viewing_mode == "cinema":
        ...
        return (
            "שמרתי את ההעדפות שלך לקולנוע, אבל כרגע החיפוש העדכני דרך Gemini לא זמין "
            "בגלל מגבלת שימוש זמנית או עומס במכסה. נסי שוב בעוד כמה דקות, או בדקי שה־API key והמכסה פעילים."
        )
