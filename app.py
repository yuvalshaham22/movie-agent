...
MOVIE_DOMAIN_TERMS = {
    ...
    "genre", "runtime", "rating", "recommend", "recommendation", "plot", "review",
    "סרט", "סרטים", "קולנוע", "שחקן", "שחקנית", "שחקנים", "במאי", "קאסט", "זאנר", "ז׳אנר",
    "אורך", "דקות", "דירוג", "המלצה", "תמליץ", "תמליצי", "עלילה", "תקציר", "ביקורת",
}
...
def normalize_user_text(text: str) -> str:
    ...
    typo_map = {"עימה": "אימה", "איימה": "אימה", "אימא": "אימה", "קומדייה": "קומדיה"}
    for wrong, correct in typo_map.items():
        updated = re.sub(re.escape(wrong), correct, updated, flags=re.IGNORECASE)
    return updated
...
def extract_requested_actor(user_text: str) -> Optional[str]:
    ...
    # Hebrew: עם אדם סנדלר / שחקן אדם סנדלר / בכיכוב אדם סנדלר
...
def extract_runtime_filter(text: str) -> Tuple[int, int]:
    ...
    if "קצר" in text or "short" in text:
        ...
        min_runtime = 120
    elif has_under and numbers:
        max_runtime = numbers[0]
    elif has_over and numbers:
        min_runtime = numbers[0]
    elif numbers and any(term in text for term in ["אורך", "דקות", "minutes", "runtime", "length", "duration"]):
        target = numbers[0]
...
def is_movie_related(user_text: str) -> bool:
    ...
    return bool(tokens.intersection(MOVIE_DOMAIN_TERMS) or detect_requested_genres(text) or find_title_in_message(text))


def has_recommendation_intent(user_text: str) -> bool:
    text = normalize_user_text(user_text).lower()
    if detect_requested_genres(text) or extract_requested_actor(text) or extract_runtime_filter(text) != (0, 1000):
        return True
    return any(term in text for term in [
        "סרט", "סרטים", "המלצה", "תמליץ", "תמליצי", "בא לי", "רוצה", "אשמח",
        "recommend", "recommendation", "movie", "film"
    ])
...
def is_detail_question(user_text: str) -> bool:
    text = normalize_user_text(user_text).lower()
    return any(term in text for term in [
        "מי משחק", "מי השחקנים", "שחקנים", "קאסט", "cast", "actor", "actors",
        "מי ביים", "במאי", "director", "על מה", "תקציר", "plot", "about", "שנת יציאה", "release",
...
def gemini_movie_fallback(user_message: str, task_name: str, cinema: bool = False) -> str:
    search_instruction = """
If the user chose cinema, use current cinema / now-playing knowledge when possible, but keep the constraints mandatory.
""" if cinema else "Do not discuss platform availability. Just recommend movies that match the request."
...
Give 3-5 concise recommendations. For each movie include: title in English, short reason, runtime if known, and style/genre.
{search_instruction}
...
def get_movie_info(user_text: str) -> Tuple[str, str]:
    ...
    title = find_title_in_message(user_text)
    ...
    return format_movie(row), "data_found"
...
def answer_recommendation(user_text: str, session_id: str, viewing_mode: str) -> str:
    # Always try the local CSV first.
    # For home viewing, CSV results are enough.
    # For cinema viewing, the CSV can validate filters but cannot verify current screenings,
    # so current cinema availability is completed by Gemini behind the scenes.
    local_answer, status = recommend_from_csv(user_text)
    save_last_context(session_id, user_text, viewing_mode)

    if viewing_mode == "home" and status == "data_found":
        return local_answer

    if viewing_mode == "cinema":
        cinema_request = user_text
        if status == "data_found":
            cinema_request += "\nCandidate movies that match the user filters from the internal movie list:\n" + local_answer
        return gemini_movie_fallback(cinema_request, "current cinema movie recommendation", cinema=True)

    # Gemini fallback is internal and invisible to the user.
...
def answer_movie_detail(user_text: str, session_id: str) -> str:
    # First try local CSV.
    local_answer, status = get_movie_info(user_text)
    if status == "data_found":
        save_last_context(session_id, user_text)
        return local_answer

    context = get_last_context(session_id)
    full_request = f"Context from previous movie conversation: {context}\nCurrent question: {user_text}" if context else user_text
    return gemini_movie_fallback(full_request, "movie information", cinema=False)


def unrelated_response() -> str:
    return (
        "אני מתמחה כאן בהמלצות סרטים וניתוח מידע על סרטים 🎬\n"
        "אפשר לשאול אותי למשל: ‘סרט קומדיה’, ‘סרט עם Adam Sandler מעל 90 דקות’, "
        "‘מי משחק בבחזרה לעתיד?’ או ‘סרט שמתאים לילדים’."
    )
...
def movie_agent(user_message: str, session_id: str = "local") -> str:
    ...
    # 2. Detail questions should not trigger home/cinema.
    if is_detail_question(original_message):
