...
def should_ask_home_or_cinema(user_text: str) -> bool:
    """Ask home/cinema only for broad genre requests.

    If the user already gave concrete constraints such as runtime, rating,
    actor, or style details, we should answer from the local CSV immediately
    instead of asking another question. Example: "סרט מצחיק עד 90 דקות"
    is already specific enough, so it should return Comedy movies under 90 minutes.
    """
    text = normalize_user_text(user_text).lower()
    if not has_recommendation_intent(text):
        return False
    ...
    if any(term in text for term in ["בבית", "טלוויזיה", "סטרימינג", "home", "streaming"]):
        return False
    if any(term in text for term in ["דומה", "כמו", "similar", "trend", "cluster", "דירוג של", "במאי"]):
        return False

    # If the request already contains actionable filters, answer directly from CSV.
    has_runtime_filter = extract_runtime_filter(text) != (0, 1000)
    has_rating_filter = extract_min_rating(text) is not None
    has_actor_filter = extract_requested_actor(text) is not None
    has_extra_style = any(term in text for term in ["קליל", "כבד", "מפחיד", "משפחתי", "ילדים", "נוער", "גיל", "מתאים"])
    if has_runtime_filter or has_rating_filter or has_actor_filter or has_extra_style:
        return False
...
3. Preserve runtime constraints from the full context. If the user asked for under two hours / less than 120 minutes / up to 90 minutes, do NOT recommend movies longer than that unless you clearly say you could not verify a matching option.
4. Preserve age constraints. If the user asks for under 18 / children / family suitability, mention age suitability carefully and avoid unsuitable horror/thriller if it is likely adult-only.
5. If you cannot verify enough currently playing movies that match ALL constraints, say that clearly and suggest checking the local cinema website or asking for a city/cinema chain. Do NOT fill the list with unrelated movies.
6. Answer in natural Hebrew for the end user. Do NOT use headings like "למה מתאים", "למה זה מתאים לבקשה", or internal reasoning.
7. For each movie use this friendly format:
🎬 Movie name
תקציר קצר: one or two friendly sentences.
סגנון: genre/style.
הערת גיל/זמינות: only if relevant.
8. Add a short note that showtimes and availability depend on the local cinema.
""".strip()
...
def movie_agent(user_message: str, session_id: str = "default") -> str:
    ...
    if pending_viewing_request:
        ...
        if preference == "home":
            ...
            # Use the full pending request plus the current follow-up, so constraints
            # from the first message are never lost after the user answers "בבית".
            local, status = recommend_by_text(combined_request)
            if status == "data_found":
                save_last_recommendations(session_id, local)
            return final_answer(combined_request, local, "home viewing recommendation from local CSV", status, allow_general_knowledge=(status == "data_missing"))
