...
def detect_viewing_preference_local(user_text: str) -> str:
    """Detect an explicit home/cinema answer without asking Gemini.

    This prevents a short answer like "בבית" from being misrouted to the
    current-cinema flow just because the previous prompt mentioned cinema.
    """
    text = normalize_user_text(user_text).lower().strip()

    home_terms = [
        "בבית", "בית", "לצפייה בבית", "צפייה בבית", "סטרימינג",
        "טלוויזיה", "טלויזיה", "home", "streaming", "watch at home"
    ]
    cinema_terms = [
        "בקולנוע", "קולנוע", "הקרנה", "הקרנות", "מוקרנים", "מוקרן",
        "עכשיו בקולנוע", "cinema", "theater", "theatre", "now playing"
    ]

    # Explicit home wins over context. This is the main bug fix.
    if any(term in text for term in home_terms):
        return "home"
    if any(term in text for term in cinema_terms):
        return "cinema"
    return "unclear"


def answer_home_recommendation(session_id: str, request_text: str) -> str:
    """Answer a home-viewing recommendation from the CSV first.

    If the CSV cannot satisfy all filters, fall back to Gemini general movie
    knowledge, not to Gemini Search/current-cinema flow.
    """
    save_last_home_request(session_id, request_text)
    local, status = recommend_by_text(request_text)
    if status == "data_found":
        save_last_recommendations(session_id, local)
        return final_answer(request_text, local, "home viewing recommendation from local CSV", status)
    return final_answer(
        request_text,
        local,
        "home viewing recommendation with Gemini general fallback",
        status,
        allow_general_knowledge=True,
    )
...
def movie_agent(user_message: str, session_id: str = "default") -> str:
    ...
    if pending_viewing_request:
        # First handle the explicit answer locally. Do NOT let Gemini infer that
        # "בבית" needs current external info only because the previous prompt
        # mentioned cinema as one of the options.
        local_preference = detect_viewing_preference_local(original_message)
        if local_preference == "home":
            full_home_request = f"{pending_viewing_request}. Viewing preference: home. User follow-up: {original_message}"
            return answer_home_recommendation(session_id, full_home_request)

        if local_preference == "cinema":
            full_cinema_request = f"{pending_viewing_request}. Viewing preference: cinema. User follow-up: {original_message}"
    ...
    if active_context:
        local_preference = detect_viewing_preference_local(original_message)
        preference = local_preference if local_preference != "unclear" else routing.get("viewing_preference", "unclear")
        normalized_current = routing.get("normalized_query_hebrew") or original_message
        if preference == "home":
            full_home_request = f"{active_context}. Viewing preference: home. User follow-up: {original_message}"
            return answer_home_recommendation(session_id, full_home_request)
        if preference == "cinema" or (local_preference != "home" and routing.get("needs_current_external_info")):
