...
# ----------------------------------------------------------------------
# 7. Conversational flow: local CSV first, Gemini/Search only when needed
# ----------------------------------------------------------------------

CINEMA_KEYWORDS = [
    "קולנוע", "בקולנוע", "לקולנוע", "להיכנס לקולנוע",
    "הקרנה", "הקרנות", "להקרנה", "מוקרן", "מוקרנים", "מוקרנות",
    "עכשיו בקולנוע", "סרטים חדשים", "סרטים בקולנוע", "ללכת לסרט", "לצאת לסרט",
    "סינמה", "בסינמה", "cinema", "theater", "theatre", "now playing", "in theaters", "in cinemas"
]

HOME_KEYWORDS = [
    "בית", "בבית", "טלוויזיה", "בטלוויזיה", "סטרימינג", "נטפליקס", "דיסני", "ביתית",
    "home", "streaming", "tv", "watch at home"
]

RECOMMENDATION_KEYWORDS = [
    "recommend", "recommendation", "movie", "film", "בא לי", "רוצה", "תמליץ", "תמליצי",
    "המלצה", "סרט", "סרטים", "משהו לראות"
]
...
def clear_state_key(session_id: str, key: str) -> Optional[str]:
    return CONVERSATION_STATE.get(session_id, {}).pop(key, None)


def has_cinema_intent(text: str) -> bool:
    msg = normalize_user_text(text).lower()
    return any(keyword in msg for keyword in CINEMA_KEYWORDS)


def has_home_intent(text: str) -> bool:
    msg = normalize_user_text(text).lower()
    return any(keyword in msg for keyword in HOME_KEYWORDS)


def has_recommendation_intent(text: str) -> bool:
    msg = normalize_user_text(text).lower()
    return any(keyword in msg for keyword in RECOMMENDATION_KEYWORDS) or len(detect_requested_genres(text)) > 0


def is_analysis_or_specific_info_request(text: str) -> bool:
    msg = normalize_user_text(text).lower()
    markers = [
        "similar", "דומה", "דומים", "כמו", "trend", "trends", "מגמות", "דפוסים",
        "anomaly", "anomalies", "חריג", "חריגות", "cluster", "קלאסטר", "אשכול",
        "ביקורת", "ביקורות", "דירוג של", "who directed", "runtime", "תקציר", "במאי", "עלילה"
    ]
    return any(marker in msg for marker in markers)


def should_ask_home_or_cinema(text: str) -> bool:
    """Ask the user where they want to watch only for broad recommendation requests."""
    if has_home_intent(text) or has_cinema_intent(text):
        return False
    if is_analysis_or_specific_info_request(text):
        return False
    return has_recommendation_intent(text)


def save_viewing_mode_pending(session_id: str, original_request: str) -> None:
    state = get_state(session_id)
    state["pending_viewing_mode_request"] = original_request


def save_last_recommendation_request(session_id: str, original_request: str) -> None:
    state = get_state(session_id)
    state["last_recommendation_request"] = original_request


def get_last_recommendation_request(session_id: str) -> str:
    return CONVERSATION_STATE.get(session_id, {}).get("last_recommendation_request", "")
...
def get_last_cinema_request(session_id: str) -> str:
    return CONVERSATION_STATE.get(session_id, {}).get("last_cinema_request", "")


def get_last_viewing_mode(session_id: str) -> str:
    return CONVERSATION_STATE.get(session_id, {}).get("last_viewing_mode", "")


def is_contextual_refinement(user_message: str) -> bool:
    """Detect follow-up constraints that should update the previous movie request.

    Examples: "שיהיה לילדים", "מתחת לגיל 18", "עד שעתיים",
    "עם שחקן מוכר". These are not new standalone requests; they refine
    the previous recommendation flow.
    """
    msg = normalize_user_text(user_message).lower().strip()
    refinement_markers = [
        "שיהיה", "שיתאים", "מתאים", "הסרט", "הסרטים", "מתחת לגיל", "מעל גיל",
        "לילדים", "לנוער", "משפחה", "עד גיל", "לא מפחיד מדי", "פחות מפחיד",
        "קצר", "ארוך", "עד", "מעל", "פחות", "דקות", "דירוג", "עם", "בלי",
        "under", "over", "less than", "kids", "family", "age", "rating", "with", "without"
    ]
    return any(marker in msg for marker in refinement_markers)


def pop_viewing_mode_pending(session_id: str) -> Optional[str]:
    return clear_state_key(session_id, "pending_viewing_mode_request")
...
def answer_summary_followup(msg, session_id):
    return None



def extract_json_object(text: str) -> Optional[dict]:
    """Extract a JSON object from a Gemini response safely."""
    if not text:
        return None
    cleaned = text.strip()
...
def analyze_request_with_gemini(user_message: str) -> Optional[dict]:
    """Ask Gemini to understand the user's intent, including Hebrew typos.

    This is only for routing. The recommendation itself still uses the local CSV first,
    unless Gemini says the request needs current external information such as movies
    ...
    prompt = f"""
You are an intent router for a movie recommendation agent.
The user may write in Hebrew, English, or with typos/transliteration.
Your job is ONLY to classify the request, not answer it.

Return JSON only with this exact schema:
{{
  "is_movie_related": true/false,
  "needs_current_external_info": true/false,
  "reason": "cinema_now_playing" | "new_releases" | "showtimes" | "local_csv_ok" | "unrelated",
  "normalized_query_hebrew": "short normalized Hebrew version of the user's request"
}}

Rules:
- Use needs_current_external_info=true if the user asks about movies currently in cinemas/theaters, screenings, showtimes, now playing, new releases, or anything that changes over time.
- Understand typos yourself. For example Hebrew misspellings of קולנוע should still count as cinema/theater intent.
- Use needs_current_external_info=false for normal historical recommendations, similar movies, runtime, rating, cast, director, trends, clusters, or anomalies from the CSV.
- If it is not related to movies, is_movie_related=false.
...
""".strip()
    ...
    try:
        ...
        analysis = extract_json_object(getattr(response, "text", ""))
        if not isinstance(analysis, dict):
            return None
        return analysis
    except Exception as exc:
        print(f"Gemini routing analysis failed: {exc}")
        return None


def get_routing_analysis(user_message: str) -> dict:
    """Gemini-first intent understanding, with a small safe local fallback."""
    analysis = analyze_request_with_gemini(user_message)
    if analysis:
        return {
            "is_movie_related": bool(analysis.get("is_movie_related", False)),
            "needs_current_external_info": bool(analysis.get("needs_current_external_info", False)),
            "reason": str(analysis.get("reason", "local_csv_ok")),
            "normalized_query_hebrew": str(analysis.get("normalized_query_hebrew", user_message)),
            "source": "gemini_router",
        }

    # Fallback only for when Gemini routing is unavailable.
    return {
        "is_movie_related": is_movie_related(user_message),
        "needs_current_external_info": has_cinema_intent(user_message),
        "reason": "cinema_now_playing" if has_cinema_intent(user_message) else "local_csv_ok",
...
def build_cinema_search_prompt(user_request: str) -> str:
    return (
        "REQUIRED: Use google_search to answer with current information. "
        "The user wants movies that are currently playing in cinemas / theaters and match the request.\n"
        f"User request: {user_request}\n"
        "Answer in Hebrew, naturally and concisely. Give 3-5 relevant movie options if possible. "
        "Prefer currently playing / now-playing / recent theatrical releases. "
        "Use this exact user-facing style for each movie: title, 'תקציר קצר:', 'סגנון:', and optionally 'הערת גיל:' only when the user asked about age/kids suitability. "
        "Do NOT use phrases/headings like 'למה מתאים', 'למה זה מתאים', 'למה זה מתאים לבקשה', 'why it matches', 'reason', 'because it fits', or any internal reasoning labels. "
        "Instead of explaining the match, give a short pleasant summary of the movie and useful user-facing details. "
        "Do NOT infer age restrictions such as 'מתחת לגיל 18' unless the user explicitly asks for family/kids/age suitability. "
        "If the user asks for age suitability, say that final age rating should be checked on the local cinema website. "
        "If you are unsure about local availability, say so transparently and suggest checking the local cinema website. "
        "Do not offer old CSV movies unless they are actually currently playing in cinemas. "
        "Do not mention internal code, routing, CSV, local dataset limitations, or API details."
    )


def clean_cinema_response(text: str) -> str:
    """Final safety cleanup so Gemini's answer stays user-facing and not internal."""
    if not text:
        return text
    replacements = {
        ...
        "Why it matches the request": "Short summary",
        "why it matches the request": "short summary",
    ...
    # Clean common bold markdown labels after replacement.
    cleaned = re.sub(r"\*\*תקציר קצר\*\*\s*:", "**תקציר קצר:**", cleaned)
    cleaned = re.sub(r"\*\*תקציר קצר:\*\*", "**תקציר קצר:**", cleaned)
...
def query_gemini_with_web_search(user_message: str) -> str:
    ...
    if not api_key or genai is None:
        return (
            "לא מצאתי את זה בדאטה המקומי, וכדי לבדוק סרטים שמוקרנים עכשיו צריך חיבור פעיל ל-Gemini. "
    ...
    try:
        ...
        return "לא התקבלו תוצאות חיפוש זמינות כרגע. נסי שוב בעוד רגע."
    except Exception as e:
        print(f"Error querying Gemini with search: {e}")
        return "מצטערת, חלה שגיאה בניסיון לבדוק מידע עדכני על סרטים בקולנוע. נסי שוב בעוד רגע."
...
def movie_agent(user_message: str, session_id: str = "default") -> str:
    ...
    # If the agent previously asked "home or cinema", use the user's answer.
    pending_viewing_request = pop_viewing_mode_pending(session_id)
    if pending_viewing_request:
        combined_request = f"{pending_viewing_request}. Viewing preference: {original_message}"
        answer_routing = get_routing_analysis(original_message)

        # Gemini routing is used here so spelling mistakes like "קלונוע" are handled by Gemini,
        # not by hard-coded typo lists in our code.
        if has_cinema_intent(original_message) or answer_routing.get("needs_current_external_info"):
        ...
        if has_home_intent(original_message):
            state = get_state(session_id)
        ...
        # keep the original request and ask again.
    ...
    # Gemini understands the user's intent first, including Hebrew typos.
    # The actual answer still uses the CSV first, except for current external info like cinema screenings.
    routing = get_routing_analysis(original_message)
    ...
    # Broad recommendation: ask the user whether they want home/CSV or cinema/current movies.
