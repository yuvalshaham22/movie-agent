...
def find_movie_title_in_message(message: str) -> Optional[str]:
    ...
    patterns = [
        ...
        r"דומים ל\s+(.+)", r"דומה ל\s+(.+)", r"כמו\s+(.+)", r"האורך של\s+(.+)",
...
def movie_agent(user_message: str, session_id: str = "default") -> str:
    ...
    if msg in ["hi", "hello", "hey", "שלום", "היי", "הי"]:
        local = (
            ...
            "אפשר לדבר איתי בשפה חופשית, גם אם יש שגיאת כתיב קטנה.\n"
            "אני מחפש קודם בדאטה המקומי, ואם חסר מידע וה־Gemini API מחובר — Gemini ישלים מהידע הכללי שלו.\n\n"
            "דוגמאות:\n"
    ...
    if any(term in msg for term in ["runtime", "length", "duration", "minutes", "short", "long", "דקות", "קצר", "ארוך", "מתחת", "פחות", "עד", "מעל"]):
        ...
        if has_filter_word:
            ...
            if status == "data_found":
                save_last_recommendations(session_id, local)
            return final_answer(original_message, local, "runtime recommendation", status, allow_general_knowledge=(status == "data_missing"))
