...
def is_movie_detail_followup(user_text: str) -> bool:
    ...
    text = normalize_user_text(user_text).lower()
    detail_terms = [
        "מי השחקנים", "שחקנים", "קאסט", "cast", "actors", "actor",
...
Rules:
1. Infer the movie title from the current question and conversation context. Hebrew titles may be local translations.
...
def movie_agent(user_message: str, session_id: str = "default") -> str:
    ...
    if active_context:
        ...
        if is_contextual_refinement(original_message):
            ...
            if mode == "cinema":
                save_last_cinema_request(session_id, refined_request)
                return query_gemini_with_web_search(build_cinema_search_prompt(refined_request))
    ...
    if any(term in msg for term in ["review", "reviews", "rating", "ratings", "who directed", "director", "plot", "about", "cast", "summary", "synopsis", "תקציר", "ביקורות", "ביקורת", "דירוג", "במאי", "עלילה", "שחקנים", "מי השחקנים", "סרטי המשך", "שנת יציאה"]):
        local, status = get_movie_info(original_message)
        if status == "data_found":
            return final_answer(original_message, local, "movie information from local CSV", status)
        return query_gemini_with_web_search(build_movie_detail_search_prompt(original_message, build_context_for_router(session_id)))
