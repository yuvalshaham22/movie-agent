...
CINEMA_KEYWORDS = [
    "קולנוע", "בקולנוע", "הקרנה", "הקרנות", "מוקרן", "מוקרנים", "מוקרנות",
    "עכשיו בקולנוע", "סרטים חדשים", "סרטים בקולנוע", "cinema", "theater", "theatre", "now playing"
]
...
def build_cinema_search_prompt(user_request: str) -> str:
    return (
        "REQUIRED: Use google_search to answer. "
        "The user wants movies that are currently playing in cinemas / theaters and match the request.\n"
        f"User request: {user_request}\n"
        "Answer in Hebrew. Give 3-5 relevant movie options if possible. "
        "For each movie include: title, why it matches the request, genre/style, and a short note that showtimes depend on the user's local cinema. "
        "Do not mention internal code or local dataset limitations."
    )
...
def movie_agent(user_message: str, session_id: str = "default") -> str:
    ...
    # Current cinema / now-playing requests are not in the historical CSV, so they go to Gemini Search.
    if has_cinema_intent(original_message):
        return query_gemini_with_web_search(build_cinema_search_prompt(original_message))
