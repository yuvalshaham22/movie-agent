...
def should_ask_home_or_cinema(user_text: str) -> bool:
    ...
    if any(term in text for term in ["בבית", "טלוויזיה", "סטרימינג", "home", "streaming"]):
        return False
    if any(term in text for term in ["דומה", "כמו", "similar", "trend", "anomaly", "cluster", "דירוג", "במאי"]):
        return False
...
""".strip()
    ...
    # Backup only if Gemini routing is unavailable.
    text = normalize_user_text(user_message).lower()
    context = normalize_user_text(conversation_context).lower()
    combined = f"{context} {text}"
    cinema_like = any(term in combined for term in ["קולנוע", "הקרנה", "מוקרנים", "עכשיו", "cinema", "theater", "theatre", "now playing"])
    home_like = any(term in text for term in ["בבית", "טלוויזיה", "סטרימינג", "home", "streaming"])
    genres = detect_requested_genres(combined)
    return {
        "is_movie_related": bool(is_movie_related(combined) or cinema_like or home_like or genres),
        "viewing_preference": "cinema" if cinema_like else ("home" if home_like else "unclear"),
        "needs_current_external_info": bool(cinema_like),
